...
const OrdersPage: React.FC = () => {
    ...
    const [sizeOptions, setSizeOptions] = useState<{ name: string; talla?: string; sku?: number }[]>([]);
    ...
    const handleSelectProduct = async (product: Product) => {
        ...
        setSearchResults([]);
        // Load sizes for selected product and populate combo
        try {
            const res = await fetch(`/api/items?productId=${product.id}`, { cache: 'no-store' });
            if (res.ok) {
                const json = await res.json();
                const arr = Array.isArray(json) ? json : (json.data || []);
                const opts = arr.map((i: any) => ({ name: String(i.name || i.talla || ''), talla: i.talla, sku: i.sku }));
                setSizeOptions(opts);
                if (opts.length === 1) setSize(opts[0].name);
            } else {
                setSizeOptions([]);
            }
        } catch {
    ...
    const handleAddToCart = () => {
        ...
        if (sizeOptions.length > 0 && !size.trim()) {
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-2 items-center relative">
                           ...
                           <div className="md:col-span-2">
                                {sizeOptions.length > 0 ? (
                                    <select value={size} onChange={e => setSize(e.target.value)} className="w-full border border-gray-300 rounded-lg p-2.5 text-sm">
                           ...
                           <div className="md:col-span-2">
                                <button onClick={handleAddToCart} disabled={!!selectedProduct && sizeOptions.length > 0 && !size.trim()} className={`w-full font-bold py-2.5 rounded-lg h-[44px] ${((!!selectedProduct && sizeOptions.length > 0 && !size.trim()) ? 'bg-gray-400 text-white cursor-not-allowed' : 'bg-red-600 text-white hover:bg-red-700')}`}>Agregar</button>
                           </div>
                        </div>
                    </div>
                    ...
                    <div className="sticky top-24 space-y-6">
                        <div className="bg-gray-100 p-4 rounded-lg shadow-sm text-sm">
                             <div className="flex justify-between items-center">
                                ...
                                <p className="font-bold text-red-600">S/ 1000.00</p>
                             </div>
